...
const DailyQuote = ({ userId, initialQuote }) => {
  ...
  return (
  <div className="bg-gradient-to-r from-green-800 to-green-900 rounded-xl shadow-md p-6 text-green-100">
    <div className="flex justify-between items-start mb-4">
      <h3 className="text-xl font-semibold">Daily Inspiration</h3>
      <button
        onClick={fetchNewQuote}
        disabled={loading}
        className="text-sm bg-green-700 bg-opacity-50 rounded-full p-2 hover:bg-opacity-70 transition-all disabled:opacity-50"
        title="Get new quote"
      >
        {loading ? (
    ...
    {quote && quote.text ? (
      <>
        ...
        {quote.author && (
          <p className="text-right text-green-300">— {quote.author}</p>
        )}
      </>
    ) : (
